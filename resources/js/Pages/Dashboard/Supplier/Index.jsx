...
const Edit = ({ auth, mustVerifyEmail, status, companies, suppliers }) => {
    const [confirmingUserDeletion, setConfirmingUserDeletion] = useState(false)
    const [supplierToDelete, setSupplierToDelete] = useState(null)
    const {delete: destroy, processing} = useForm()
    const {data, current_page, total, links, from, to} = suppliers

    useEffect(() => {
        setConfirmingUserDeletion(!!supplierToDelete)
    }, [supplierToDelete]);

    const handleDelete = (id) => {
        destroy(route('supplier.destroy', supplierToDelete.id))
                            ...
                            columns={[
                                ...
                                {name: 'Nome', sortable: true, selector: row => row.short_name},
                                {
                                    cell: (d) => [
                                        ...
                                        <button onClick={() => setSupplierToDelete(d)} className="mx-2 text-red-500">Excluir</button>
                                    ]
                                }
                            ]}
                            data={data}
                ...
                <div className="p-6">
                    ...
                    <div className="mt-1 text-sm text-gray-600 dark:text-gray-400 mb-6">
                        <p className="text-sm text-gray-500">
                            Tem certeza que deseja deletar o fornecedor: {supplierToDelete?.name}? Esta ação não pode ser desfeita.
                        </p>
                    </div>

                    <div className="flex justify-end">
                        <SecondaryButton className="mx-2" onClick={() => setSupplierToDelete(null)}>Cancelar</SecondaryButton>
